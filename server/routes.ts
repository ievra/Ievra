...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/og-asset/*", async (req, res) => {
  ...
  app.get("/robots.txt", (req, res) => {
    ...
    res.send([
      'User-agent: *',
      '',
      '# === PUBLIC APIs — required for Googlebot JS rendering ===',
      'Allow: /api/projects',
      'Allow: /api/projects/',
      'Allow: /api/articles',
      'Allow: /api/articles/',
      ...
      'Allow: /api/advantages',
      'Allow: /api/journey-steps',
      'Allow: /api/lookup',
      'Allow: /api/about-content',
      'Allow: /api/about-page-content',
      'Allow: /api/about-showcase-services',
      'Allow: /api/about-process-steps',
      'Allow: /api/about-team-members',
      'Allow: /api/about-awards',
      'Allow: /api/about-core-values',
      '',
      '# === PRIVATE APIs — block crawlers ===',
      'Disallow: /api/auth/',
      'Disallow: /api/upload',
      'Disallow: /api/admin/',
      'Disallow: /api/users/',
      ...
      'Disallow: /api/bp-categories/',
      'Disallow: /api/bp-statuses/',
      'Disallow: /api/bp-tiers/',
      'Disallow: /api/bp-transactions/',
      'Disallow: /api/crm-pipeline-stages/',
      'Disallow: /api/crm-customer-tiers/',
      'Disallow: /api/crm-statuses/',
      'Disallow: /api/interactions/',
      'Disallow: /api/deals/',
      'Disallow: /api/transactions/',
      'Disallow: /api/warranty-logs/',
      'Disallow: /api/construction-phases/',
      'Disallow: /api/design-phases/',
      'Disallow: /api/dashboard/',
      'Disallow: /api/debug-paths',
      'Disallow: /admin',
      'Disallow: /login',
      '',
...
<style>
html,body{margin:0;padding:0;background:#1e1e1e;color:#d4d4d4;font-family:'Consolas','Courier New',monospace;font-size:13.5px;line-height:1.6}
...
.url-block{margin:4px 0}
.i1{padding-left:20px}
.i2{padding-left:40px}
</style>
</head><body>
<div class="decl">&lt;?xml version="1.0" encoding="UTF-8"?&gt;</div>
<div><span class="tag">&lt;urlset</span> <span class="attr">xmlns</span>=<span class="val">"http://www.sitemaps.org/schemas/sitemap/0.9"</span><span class="tag">&gt;</span></div>
<xsl:for-each select="sm:urlset/sm:url">
<div class="url-block">
  <div class="i1"><span class="tag">&lt;url&gt;</span></div>
  <div class="i2"><span class="tag">&lt;loc&gt;</span><a style="color:#4fc1ff;text-decoration:none" href="{sm:loc}"><xsl:value-of select="sm:loc"/></a><span class="tag">&lt;/loc&gt;</span></div>
  <xsl:if test="sm:lastmod"><div class="i2"><span class="tag">&lt;lastmod&gt;</span><span class="txt"><xsl:value-of select="sm:lastmod"/></span><span class="tag">&lt;/lastmod&gt;</span></div></xsl:if>
  <xsl:if test="sm:changefreq"><div class="i2"><span class="tag">&lt;changefreq&gt;</span><span class="txt"><xsl:value-of select="sm:changefreq"/></span><span class="tag">&lt;/changefreq&gt;</span></div></xsl:if>
  <xsl:if test="sm:priority"><div class="i2"><span class="tag">&lt;priority&gt;</span><span class="txt"><xsl:value-of select="sm:priority"/></span><span class="tag">&lt;/priority&gt;</span></div></xsl:if>
  <div class="i1"><span class="tag">&lt;/url&gt;</span></div>
</div>
</xsl:for-each>
<div><span class="tag">&lt;/urlset&gt;</span></div>
</body></html>
</xsl:template>
</xsl:stylesheet>`);
  });
  ...
  app.get("/sitemap.xml", async (req, res) => {
    ...
    try {
      ...
      xml += `<?xml-stylesheet type="text/xsl" href="/sitemap.xsl?v=2"?>\n`;
  ...
  app.put("/api/clients/:id", requirePermission('clients', 'lookup'), async (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        console.error("Client validation errors:", JSON.stringify(error.errors, null, 2));
  ...
  app.get("/api/settings", async (req, res) => {
    try {
      ...
      console.error("[/api/settings] Error:", error?.message, error?.code);
      res.status(500).json({ message: "Failed to fetch settings", detail: error?.message });
    }
  });
  ...
  // BP Statuses routes
  app.get("/api/bp-statuses", async (req, res) => {
    try {
      const { active } = req.query;
      const filters: any = {};
      if (active !== undefined) filters.active = active === 'true';
      const statuses = await storage.getBpStatuses(filters);
      res.json(statuses);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch BP statuses" });
    }
  });
  ...
  app.post("/api/bp-statuses", requirePermission('clients'), async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to create BP status" });
    }
  });
  ...
  app.put("/api/bp-statuses/:id", requirePermission('clients'), async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to update BP status" });
    }
  });
